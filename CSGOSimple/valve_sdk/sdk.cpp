...
namespace Interfaces
{
    ...
    template<typename D>
    D* get_interfaceD(const char* modName, const char* ifaceName, bool exact = false) {
        D* iface = nullptr;
        ...
        for (InterfaceReg* cur = ifaceRegList; cur; cur = cur->m_pNext) {
            if (exact == true) {
                if (strcmp(cur->m_pName, ifaceName) == 0)
                    iface = reinterpret_cast<D*>(cur->m_CreateFn());
            } else {
    ...
    void Initialize()
    {
        ...
        g_pEffects = get_interface<CEffects>(engineFactory, "VEngineEffects001");
        ...
        g_SpatialPartition = get_interfaceD<uintptr_t>("engine.dll", "SpatialPartition");
        g_Localize = get_interfaceD<ILocalize>("localize.dll", "Localize_");
