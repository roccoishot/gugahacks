...
void LagComp::Run()
{
	for (auto i = 1; i <= g_GlobalVars->maxClients; i++)
	{
		...
		{
			auto cur_record = records[player->EntIndex()].at(records[player->EntIndex()].size());
			...
			if (fabs(delta.Length()) < 4096.f) // are they? shit. nigga.
				LagCompensate(player, cur_record, prev_record);
		}
		else
...
void LagComp::LagCompensate(C_BasePlayer* player, LagRecord record, LagRecord prev_record)
{
	// extrapolate
	{
