...
void CRageBot::CreateMove(CUserCmd* cmd, bool& bSendPacket)
{
	...
	QAngle newAng = Math::CalcAngle(g_LocalPlayer->GetEyePos(), predicted);
	QAngle a = (g_LocalPlayer->m_aimPunchAngle() * 2);
	a.roll = 0.f;
	newAng -= a;
	
	if (weapon->IsSniper() && !g_LocalPlayer->m_bIsScoped() && (g_LocalPlayer->m_fFlags() & FL_ONGROUND) && g_Options.ragebot_autoscope[curGroup])
	{
	...
	float chance = g_Options.ragebot_hitchance[curGroup];
	...
	if (!weapon->CanFire() && g_GlobalVars->curtime <= weapon->m_flNextPrimaryAttack())
		return;
...
int CRageBot::FindBestEntity(CUserCmd* cmd, C_BaseCombatWeapon* weapon, Vector& hitpos, bool& bBacktrack)
{
	...
	float minFoV = (float)g_Options.ragebot_fov;
	
	Vector ViewOffset = g_LocalPlayer->m_vecOrigin() + g_LocalPlayer->m_vecViewOffset();
	...
	for (int i = 0; i <= g_EngineClient->GetMaxClients(); i++)
	{
		...
		float fov = Math::GetFOV(viewAngles, Math::CalcAngle(g_LocalPlayer->GetEyePos(), CHitpos));

		if (fov > g_Options.ragebot_fov)
			continue;

		switch (g_Options.ragebot_selection[curGroup])
		{
		case 0:
			if (fov < minFoV)
			{
	...
	for (int hitbox = 0; hitbox < Hitboxes::HITBOX_MAX; hitbox++)
	{
		...
		case HITBOX_HEAD:
			if (!g_Options.ragebot_hitbox[0][curGroup])
				continue;
			pointscale = g_Options.ragebot_hitbox_multipoint_scale[0][curGroup];
			break;

		case HITBOX_NECK:
			if (!g_Options.ragebot_hitbox[1][curGroup])
				continue;
			pointscale = g_Options.ragebot_hitbox_multipoint_scale[1][curGroup];
			break;
		case HITBOX_PELVIS:
			if (!g_Options.ragebot_hitbox[2][curGroup])
				continue;
			pointscale = g_Options.ragebot_hitbox_multipoint_scale[2][curGroup];
			break;
		case HITBOX_STOMACH:
			if (!g_Options.ragebot_hitbox[3][curGroup])
				continue;
			pointscale = g_Options.ragebot_hitbox_multipoint_scale[3][curGroup];
			break;
		...
		case HITBOX_UPPER_CHEST:
			if (!g_Options.ragebot_hitbox[4][curGroup])
				continue;
			pointscale = g_Options.ragebot_hitbox_multipoint_scale[4][curGroup];
			break;
		...
		case HITBOX_LEFT_CALF:
			if (!g_Options.ragebot_hitbox[5][curGroup])
				continue;
			pointscale = g_Options.ragebot_hitbox_multipoint_scale[5][curGroup];
			break;

		case HITBOX_RIGHT_FOOT:
		case HITBOX_LEFT_FOOT:
			if (!g_Options.ragebot_hitbox[6][curGroup])
				continue;
			pointscale = g_Options.ragebot_hitbox_multipoint_scale[6][curGroup];
			break;
		...
		case HITBOX_LEFT_FOREARM:
			if (!g_Options.ragebot_hitbox[7][curGroup])
				continue;
			pointscale = g_Options.ragebot_hitbox_multipoint_scale[7][curGroup];
			break;
		default:
			continue;
		}
		...
		for (size_t p = 0; p < Points.size(); p++)
		{
			...
			{
				if (CDamage >= g_Options.ragebot_mindamage[curGroup] && CDamage > CanHitHead.damage)
				{
			...
			if (Points[p].center && CDamage >= g_Options.ragebot_mindamage[curGroup])
			{
				CCanHitCenter = true;
			...
			if (CDamage >= g_Options.ragebot_mindamage[curGroup] && CDamage > CBestDamage)
			{
				CBestDamage = CDamage;
		...
		if (CBestDamage >= g_Options.ragebot_mindamage[curGroup] && CanBaimKill.damage < CBestDamage && (hitbox == HITBOX_CHEST || hitbox == HITBOX_LOWER_CHEST || hitbox == HITBOX_PELVIS || hitbox == HITBOX_STOMACH))
		{
		...
		if (CBestDamage >= g_Options.ragebot_mindamage[curGroup] && CBestDamage > BestDamage)
		{
